#!/bin/bash
snapshots_to_delete=$(aws --profile oldprod ec2 describe-snapshots --owner-ids 357618474061 --query 'Snapshots[?StartTime<=`2020-06-27`].SnapshotId' --output json | tr -d '"' | tr ',' '\n' | tr -d '[]' | sed '/^$/d')
